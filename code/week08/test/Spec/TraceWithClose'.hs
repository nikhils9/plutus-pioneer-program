...
tests :: TestTree
tests = testGroup "token sale trace with close'"
    [
        tests' myPredicate myTrace,
        tests' myPredicate2 myTrace2
    ]    

tests' :: TracePredicate -> EmulatorTrace () -> TestTree
tests' predicate trace = checkPredicateOptions myOptions "token sale trace" predicate trace
